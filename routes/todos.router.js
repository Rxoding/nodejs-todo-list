...
router.post('/todos', async (req, res, next) => {
  try {
    ...
    // 1-5. 만약, 클라이언트가 value 데이터를 전달하지 않았을때 클라이언트에게 에러메세지 전달
    if (!value) {
      return res
        .status(400)
    ...
    // 2. 해당하는 마지막 order 데이터를 가져온다
    // findOne = 1개의 데이터만 조회한다.
    // sort = 정렬한다 -> 어떤 컬럼을? order을 기준으로 내림차순으로
    // exec를 사용해 promise 형태로 반환, await 활성화
    const todoMaxOrder = await Todo.findOne().sort('-order').exec();

    // 3. 만약 존재한다면 현재 해야할 일을 +1 하고, order 데이터가 존재하지 않는다면 1로 할당
    const order = todoMaxOrder ? todoMaxOrder.order + 1 : 1;

    // 4. 해야할 일 등록
    const todo = new Todo({ value, order }); // todo를 실제 인스턴스 형식으로 만듬
    await todo.save(); // 실제로 db에 저장
...
router.patch('/todos/:todoId', async (req, res, next) => {
  ...
  // 현재 나의 todoId(order)가 무엇인지 알아야한다.
